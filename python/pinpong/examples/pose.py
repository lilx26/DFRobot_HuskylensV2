...
def draw_result(result):
    ...
    if result:
        ...
        for name, (x, y) in keypoints.items():
            draw_point(x, y, color="red", label=name)
        ...
        for p1, p2 in skeleton:
            draw_line(keypoints[p1], keypoints[p2])
  ...
  def run(self):
    while True:
      ...
      while True:
        ...
        result = husky.getCachedCenterResult(ALGORITHM_POSE_RECOGNITION)
        
        if result:
            print(f"nose{result.nose_x,result.nose_y}")
            pass
        else:
            print(None)
